...
function generatePDFReport() {
  ...
  const deptBudget = {};
  projects.forEach(p => { const d = p.department; if (!deptBudget[d]) deptBudget[d] = {count:0, budget:0}; deptBudget[d].count++; deptBudget[d].budget += getBudgetBase(p); });
  ...
  const anomalyProjects = projects
    .map(p => ({ p, rate: Math.abs(getChangeRate(p) || 0), change: getChangeAmount(p) || 0, budget: getBudgetBase(p) }))
...
'<div class="cover"><h1>${window.BASE_YEAR}년 AI 재정사업 분석 보고서</h1>' +
'<div class="subtitle">' + (meta.source || '${window.BASE_YEAR}년 AI 재정사업 현황') + '</div>' +
...
mkT(['사업명','부처',`${window.BASE_YEAR} 예산`,'증감액','증감률(%)'], anomalyProjects.map(x => [
...
mkT(['사업명','부처',`${window.BASE_YEAR} 예산`,'위험점수','등급'], riskProjects.map(x => {
  const grade = getWasteRiskGrade(x.score);
  return [(x.p.project_name||x.p.name).substring(0,30), x.p.department, fmtB(getBudgetBase(x.p)), x.score, grade.label];
})) +

'<div class="page-break"></div>' +
'<h2>5. 중복 의심 사업 그룹 (' + Math.min(dups.length, 16) + '개)</h2>' +
dups.slice(0, 16).map((g, i) => '<div style="margin-bottom:10px;page-break-inside:avoid"><strong>' + (i+1) + '. ' + g.group_name + '</strong> (' + (g.project_count || g.projects.length) + '개 사업, ' + fmtB(g.total_budget || 0) + ', 유사도: ' + (g.grade || '-') + ')<ul style="margin:2px 0 0;padding-left:20px;font-size:11px">' + g.projects.slice(0, 6).map(p => '<li>' + p.department + ': ' + p.name + ' (' + fmtB(p.budget_base || 0) + ')</li>').join('') + '</ul></div>').join('') +

'<div class="page-break"></div>' +
'<h2>6. 전체 사업 목록 (부록)</h2>' +
mkT(['번호','사업명','부처',`${window.BASE_YEAR} 예산`,'증감률(%)','유형'], projects.slice().sort((a,b) => getBudgetBase(b) - getBudgetBase(a)).map((p, i) => [
  i+1,
  (p.project_name||p.name).substring(0, 35),
  p.department,
  fmtB(getBudgetBase(p)),
...
'${window.BASE_YEAR}년 AI 재정사업 분석 플랫폼 자동 생성 PDF 보고서 | ' + new Date().toLocaleDateString('ko-KR') +
'</div></body></html>';
...
function generateMarkdownReport() {
  ...
  let md = '';
  md += '# ${window.BASE_YEAR}년 AI 재정사업 분석 보고서\n\n';
  md += '> ' + (meta.source || '${window.BASE_YEAR}년 AI 재정사업 현황') + '\n\n';
  md += '- **분석일**: ' + date + '\n';
  ...
  md += '| 순위 | 부처명 | 사업 수 | ${window.BASE_YEAR} 예산(백만원) | 비중(%) |\n';
  md += '|------|--------|---------|-------------------|--------|\n';
  ...
  md += '| 사업명 | 부처 | ${window.BASE_YEAR} 예산 | 증감액 | 증감률(%) |\n';
  md += '|--------|------|-----------|--------|----------|\n';
  ...
  md += '| 사업명 | 부처 | ${window.BASE_YEAR} 예산 | 위험점수 | 등급 |\n';
  md += '|--------|------|-----------|----------|------|\n';
  riskProjects.forEach(x => {
    const grade = getWasteRiskGrade(x.score);
    md += '| ' + (x.p.project_name||x.p.name).substring(0,30) + ' | ' + x.p.department + ' | ' + fmtB(getBudgetBase(x.p)) + ' | ' + x.score + ' | ' + grade.label + ' |\n';
  });
  md += '\n';
  ...
  dups.slice(0, 16).forEach((g, i) => {
    ...
    g.projects.slice(0, 6).forEach(p => {
      md += '  - ' + p.department + ': ' + p.name + ' (' + fmtB(p.budget_base || 0) + ')\n';
    });
    md += '\n';
  });

  md += '---\n\n';
  md += '*${window.BASE_YEAR}년 AI 재정사업 분석 플랫폼 자동 생성 Markdown 보고서 | ' + date + '*\n';
