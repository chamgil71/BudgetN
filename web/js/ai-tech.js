...
(function () {
  ...
  `;
    ...
    sorted.forEach(([tech, data]) => {
      const top3 = [...data.projects].sort((a, b) => getBudgetBase(b) - getBudgetBase(a)).slice(0, 3);
      ...
      top3.forEach((p, i) => {
        ...
        <span style="color:var(--accent);font-weight:600;white-space:nowrap;">${formatBillion(getBudgetBase(p))}원</span>
      </div>`;
      });
      html += `</div>`;
    });
  ...
  `;
    ...
    techEfficiency.forEach(t => {
      const sorted = [...t.projects].sort((a, b) => getBudgetBase(b) - getBudgetBase(a));
      const top3Budget = sorted.slice(0, 3).reduce((s, p) => s + getBudgetBase(p), 0);
    ...
    const totalAll = projects.reduce((s, p) => s + getBudgetBase(p), 0);
    const ranges = [
    ...
    const rangeData = ranges.map(r => {
      ...
      return { ...r, count: matched.length, budget: matched.reduce((s, p) => s + getBudgetBase(p), 0) };
    });
  ...
  window.initAiTechTab = initAiTechTab;
})();
