...
function initFutureSimTab(DATA) {
  ...
  function getB25(p) { return p.budget?.budget_prev ?? p.budget?.['2025_original'] ?? 0; }
  function getB26(p) { return p.budget?.budget_base ?? p.budget?.['2026_budget'] ?? 0; }
  ...
  function calcDedupSavings(groups) {
    ...
    groups.forEach((g, i) => {
      ...
      const sorted = [...(g.projects || [])].sort((a, b) => (b.budget_base || 0) - (a.budget_base || 0));
      sorted.forEach((p, j) => {
        const b = p.budget_base || 0;
        totalOriginal += b;
        if (j > 0) {
  ...
  function renderDedupGroups(groups) {
    ...
    groups.forEach((g, i) => {
      ...
      let groupSavings = 0;
      if (checked) {
        sorted.forEach((p, j) => { if (j > 0) groupSavings += (p.budget_base || 0) * ratio; });
      }
    ...
    groups.forEach((g, i) => {
      ...
      sorted.forEach((p, j) => {
        if (j > 0) {
          ...
          deptSavings[d] += (p.budget_base || 0) * ratio;
        }
      });
    });
...
window.initFutureSimTab = initFutureSimTab;
