...
(function () {
  ...
  function _b26(p) { return p.budget?.budget_base ?? p.budget?.['2026_budget'] ?? 0; }
  ...
  function _b25(p) { return p.budget?.budget_prev ?? p.budget?.['2025_original'] ?? 0; }
  ...
  function renderPriorityMatrix(projects, classMap) {
    ...
    svg.append('text').attr('x', width / 2).attr('y', height + 35).attr('text-anchor', 'middle')
      .attr('fill', labelColor).attr('font-size', 11).text('${window.BASE_YEAR} 예산 규모');
  ...
  function generateRecommendations(projects, themeMap, classMap) {
    ...
    if (newProjects.length > 0) {
      ...
      recs.push({
        ...
        body: `${window.BASE_YEAR}년 신규 AI 사업 ${newProjects.length}건이 편성되어 총 ${_fmt(newBudget)}이 배정되었습니다. 기존 사업과의 중복 여부 및 사전 타당성 검토가 중요합니다.`,
      ...
      if (subs.length > 0) {
        subs.forEach(s => {
          const row = {
            ...
            budget2025: s.budget_prev || 0,
            budget2026: s.budget_base || 0,
  ...
  const AI_FIELDS = [
    ...
    { id: 'sub', label: '내역사업명', extract: p => (p.sub_projects || []).map(s => s.name || '').join(' '), extractDetail: p => (p.sub_projects || []).map(s => ({ name: s.name || '', budget: s.budget_base || 0 })) },
      ...
      wrap.innerHTML = `
        <table class="data-table" style="width:100%;font-size:12px">
          <thead><tr>
            ...
            <th>유형</th>
            <th class="nai-sort num" data-key="budget" style="cursor:pointer;text-align:right">${window.BASE_YEAR} 예산${si('budget')}</th>
          </tr></thead>
          <tbody>
            ${showRows.map(p => {
        const pName = p.project_name || p.name || '';
      ...
      lowWrap.innerHTML = `
        <table class="data-table" style="width:100%;font-size:12px">
          <thead><tr>
            ...
            <th style="text-align:right">${window.BASE_YEAR} 예산</th>
          </tr></thead>
          <tbody>
            ${lowSorted.map(p => {
        const pName = p.project_name || p.name || '';
